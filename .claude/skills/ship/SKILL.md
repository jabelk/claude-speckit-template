...
3. Run the local review gate against the base branch and address its findings now — the PR-side review is the safety net, not the first pass. Both legs, in this order: `review-plan-v2 --static-only --plain --base <base>` (deterministic analyzers only, nothing billed, no diff leaves the machine — **never run it without `--static-only`**; its AI reviewer legs were retired on 2026-08-28) then `coderabbit review --base <base> --include-untracked` (plain text is the default; `--plain` is not a flag and exits **1**, which is also "found actionable issues"). `--include-untracked` is mandatory at this point in the flow: this gate runs in Phase 2 and staging happens in Phase 3, so without it every file the change ADDS is silently skipped, and that omission is indistinguishable from a clean pass. Report which legs actually ran: `coderabbit auth status` has to show an account, and a `--static-only` run printing `[NOTHING REVIEWED]` or `reviewing 0 of N` checked nothing regardless of exiting 0 — that leg reads the committed diff, so commit before trusting it.
