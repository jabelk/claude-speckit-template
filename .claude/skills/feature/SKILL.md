...
Run `/ship` — the shipping mechanics (CodeRabbit-style self-review checklist, unpiped gates, PR, batched review round, merge, per-project promote/verify) live in that skill as the single source of truth. Do not inline-duplicate its steps here; if shipping behavior needs to change, change `/ship`.
